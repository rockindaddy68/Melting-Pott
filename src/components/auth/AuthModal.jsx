    ...
    try {
      ...
      if (result.success) {
        ...
        if (registerData.newsletter) {
          const newsletterResult = await userService.subscribeToNewsletter(
            registerData.email, 
            {
              eventTypes: ['music', 'theater', 'arts', 'festivals'],
              cities: [registerData.city],
              frequency: 'weekly'
            }
          );
          ...
          if (newsletterResult.success) {
            ...
            console.log('✅ Newsletter abonniert:', newsletterResult.message);
          }
        }
          ...
          {!isLogin && (
            <div className="flex items-start space-x-3">
              <input
                ...
                className="mt-1 w-4 h-4 bg-gray-700 border border-gray-600 rounded text-orange-500 focus:ring-orange-500 focus:ring-2"
              />
              <label htmlFor="newsletter-consent" className="text-sm text-gray-300 cursor-pointer">
                📧 Ich möchte den <span className="text-orange-400 font-medium">kostenlosen Event-Newsletter</span> erhalten (wöchentlich, jederzeit kündbar)
              </label>
            </div>
          )}
          
          <p className="text-sm text-gray-400 text-center">
            Mit der {isLogin ? 'Anmeldung' : 'Registrierung'} stimmen Sie unseren{' '}
